...
/// The abstract syntax tree (AST) for a Brainfuck program.
// Not really a tree, just a list of tokens and a jump table.
//
// `jump_table[i]` will give the token idx to conditionally
// jump to for the `tokens[i]` token, only two types of
// token need jumping, the `LoopStart` and `LoopEnd`.
//
...
pub fn parse(input: &str) -> Result<Ast, ParseError> {
    ...
    for (token_pos, char) in input.char_indices() {
        ...
        let Some(token) = Token::from_char(char) else {
            continue;
        };
...
const _: () = {
    ...
    assert!(size_of::<Token>() == 1);
    assert!(align_of::<Token>() == 1);

    assert!(size_of::<Option<Token>>() == 1);
    assert!(align_of::<Option<Token>>() == 1);
};
